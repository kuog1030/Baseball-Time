...
data class Box(
    ...
    fun toBoxViewList(): List<BoxView> {
        ...
        result.add(
            BoxView(
                type = "R",
                guest = this.run[0].toString(),
                home = this.run[1].toString()
            )
        )
        result.add(
            BoxView(
                type = "H",
                guest = this.hit[0].toString(),
                home = this.hit[1].toString()
            )
        )
        result.add(
            BoxView(
                type = "E",
                guest = this.error[0].toString(),
                home = this.error[1].toString()
            )
        )
